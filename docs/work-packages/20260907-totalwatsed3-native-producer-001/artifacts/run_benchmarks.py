...
for run in ('incommensurate-stickball','throbbing-sylvan'):
 for repetition in range(1,6):
  for mode in ('python','native'):
   ...
   commands.append(cmd);(out/'benchmark-commands.json').write_text(json.dumps(commands,indent=2)+'\n')
   ...
   (out/'benchmark-measurements.json').write_text(json.dumps(records,indent=2)+'\n')

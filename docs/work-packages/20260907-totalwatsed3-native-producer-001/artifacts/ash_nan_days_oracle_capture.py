...
folder=Path(tempfile.mkdtemp(prefix='independent-review-ash-days-nan-',dir=root/'target/totalwatsed3-evidence'))
fixture=root/'tests/fixtures/totalwatsed3/contracts/ash_no_year0'
for p in fixture.glob('H.*.parquet'):
    (folder/p.name).symlink_to(p)
(folder/'ash').mkdir()
for p in (fixture/'ash').glob('*.parquet'):
    table=pq.read_table(p)
    values=table['days_from_fire (days)'].to_pylist()
    values[0]=float('nan')
    table=table.set_column(table.schema.get_field_index('days_from_fire (days)'),'days_from_fire (days)',pa.array(values,type=pa.float64()))

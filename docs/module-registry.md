...
| `wepppyo3.wepp_interchange` | production-critical | `release/linux/py312/wepppyo3/wepp_interchange/wepp_interchange_rust.so` | `wepppy/wepp/interchange/_rust_interchange.py`, watershed/hillslope output converters, catalog scan, slope segmentation, Roads pass combination, AgFields weighted PASS accounting | `tests/wepp_interchange/test_*.py`; weighted and explicit-breakpoint Rust identities; non-empty UTF-8 Arrow writer regression; ordered direct hillslope Parquet regressions; watershed TC_OUT regression; Python release-API tests; WEPPpy interchange tests | `confirmed`: native writers preserve public schemas and use Parquet dictionary encoding. `confirmed`: ordered hillslope PASS, EBE, ELEMENT, LOSS, SOIL, and WAT APIs write directly from source-ordered compact Rust arrays without returning full tables through Python; each source forms one Parquet row group. `confirmed`: watershed EBE includes native outlet inference and channel-peak regression auditing, while TC_OUT performs native outlet selection. `confirmed`: additive weighted API preserves the Roads call surface and returns serialized closure diagnostics. `confirmed`: explicit-breakpoint slope segmentation preserves the existing automatic segmenter while accepting validated field-aware OFE boundaries and an optional target width. |

...
mod tests {
    ...
    fn temp_path(stem: &str) -> PathBuf {
        ...
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        path.push(format!("swat_utils_{stem}_{nanos}.parquet"));
        path
    }
    ...
    #[test]
    fn arrow01_writer_schema_metadata_roundtrip_swat_utils() {
        let mut field_meta = HashMap::new();
        field_meta.insert("units".to_string(), "day".to_string());
        field_meta.insert("description".to_string(), "calendar value".to_string());
        let field = Field::new("value", DataType::Int32, true).with_metadata(field_meta);

        let mut dataset_meta = HashMap::new();
        dataset_meta.insert("dataset_version".to_string(), "3.0".to_string());
        dataset_meta.insert("source_file".to_string(), "calendar.parquet".to_string());
        let schema = Schema::new_with_metadata(vec![field], dataset_meta);

        let path = temp_path("schema_roundtrip");
        let summary = write_single_chunk(&path, schema, one_col_chunk(vec![Some(1), Some(2)]))
    ...
    #[test]
    fn arrow01_writer_empty_chunk_contract_swat_utils() {
    ...
    #[test]
    fn arrow01_writer_row_group_and_row_count_accounting_swat_utils() {
        ...
        sink.write_chunk(one_col_chunk(vec![Some(10), Some(11)]))
            .expect("first chunk");
        sink.write_chunk(one_col_chunk(vec![Some(12)]))
            .expect("second chunk");

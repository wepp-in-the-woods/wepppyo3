...
for count in (73,146,293,586,2344,5860):
 ...
 commands.append(cmd);(out/'review-scale-commands.json').write_text(json.dumps(commands,indent=2)+'\n')
 done=subprocess.run(cmd,capture_output=True,text=True)
 if done.returncode:
  (out/f'scale-failure-{count}.log').write_text(done.stdout+'\n'+done.stderr);raise RuntimeError(f'scale {count} failed')
 r=json.loads(done.stdout.strip().splitlines()[-1]);records.append(r);(out/'review-scale-measurements.json').write_text(json.dumps(records,indent=2)+'\n');print(count,round(r['observations'][0]['wall_seconds'],3),r['memory_peak_bytes'],flush=True)
 if count==5860:

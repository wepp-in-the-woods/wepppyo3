...
| `wepppyo3.wepp_interchange` | production-critical | `release/linux/py312/wepppyo3/wepp_interchange/wepp_interchange_rust.so` | `wepppy/wepp/interchange/_rust_interchange.py`, watershed/hillslope output converters, catalog scan, slope segmentation, Roads pass combination, AgFields weighted PASS accounting | `tests/wepp_interchange/test_*.py`; weighted Rust identities and Python release-API test; WEPPpy interchange tests | `confirmed`: default Rust paths with explicit fallback logging in WEPPpy interchange docs. `confirmed`: additive weighted API preserves the Roads call surface and returns serialized closure diagnostics. |

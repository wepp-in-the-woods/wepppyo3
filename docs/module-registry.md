...
| `wepppyo3.wepp_interchange` | production-critical | `release/linux/py312/wepppyo3/wepp_interchange/wepp_interchange_rust.so` | `wepppy/wepp/interchange/_rust_interchange.py`, watershed/hillslope output converters, catalog scan, slope segmentation, Roads pass combination, AgFields weighted PASS accounting | `tests/wepp_interchange/test_*.py`; weighted and explicit-breakpoint Rust identities; non-empty UTF-8 Arrow writer regression; direct multi-file WAT Parquet regression; Python release-API tests; WEPPpy interchange tests | `confirmed`: default Rust paths preserve public UTF-8 schemas and use Parquet dictionary encoding, with explicit fallback logging in WEPPpy interchange docs. `confirmed`: multi-OFE hillslope WAT writes directly from source-ordered compact Rust arrays without returning full tables through Python. `confirmed`: additive weighted API preserves the Roads call surface and returns serialized closure diagnostics. `confirmed`: explicit-breakpoint slope segmentation preserves the existing automatic segmenter while accepting validated field-aware OFE boundaries and an optional target width. |

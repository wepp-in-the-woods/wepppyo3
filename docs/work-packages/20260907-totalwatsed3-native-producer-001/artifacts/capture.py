...
for run in ("incommensurate-stickball","throbbing-sylvan"):
 src=Path("/workdir/wepppyo3/tests/fixtures/totalwatsed3")/run
 ...
 result=dict(run=run,baseflow=vars(opts),output=str(out),seconds=elapsed,baseline=before,memory_peak=int(Path("/sys/fs/cgroup/memory.peak").read_text()),schema_equal=actual.schema.equals(oracle.schema,check_metadata=True),rows=actual.num_rows,differences=diff)
 results.append(result)
 Path("/evidence/python-baseline.json").write_text(json.dumps(results,indent=2)+"\n")
